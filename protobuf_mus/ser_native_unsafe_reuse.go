...
func (s SerializerNativeUnsafeReuse) Marshal(data *data_proto.DataRaw) (bs []byte,
	...
	if data.Time != nil {
		...
		n += ext.TimestampNativeProtobuf.Marshal(data.Time, s.bs[n:])
	}
	bs = s.bs[:n]
	return
}
...
func (s SerializerNativeUnsafeReuse) Name() benchser.ResultName {
	return benchser.NewResultName(Protobuf, s.features()...)
}

func (s SerializerNativeUnsafeReuse) Features() []benchser.Feature {
	return append(GeneralFeatures, s.features()...)
}

func (s SerializerNativeUnsafeReuse) features() []benchser.Feature {
	return []benchser.Feature{benchser.Native, benchser.Unsafe,
		benchser.Reuse}
}
